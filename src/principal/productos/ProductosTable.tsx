...
import { useProductosStore, useUiStore } from "../../hooks";
...
export const ProductosTable = () => {
  ...
  const { toggleProducto } = useUiStore();
  ...
  const openUpdateProductoModal = (producto: IProductos) => {
    setActiveProducto(producto);
    toggleProducto();
  }

  // Activate/Inactivate - Producto

  const activateInactivateProductoFnc = (producto: IProductos) => {
    ...
    activeInactiveProducto(dataUpdate);
  ...
  const filteredItems = useMemo(() => {
    ...
    let filteredElements: IProductos[] = [...productos];
    ...
    filteredElements = filteredElements.filter((element: any) =>
      ...
      element.unidadMedida.descripcion.includes(filterValue.toUpperCase())
    );

    return filteredElements;

  }, [productos, filterValue, filterActiveValue]);

  // Sort handler

  const sortedItems = useMemo(() => {
    return [...filteredItems].sort((a: IProductos, b: IProductos) => {

      let first = null;
      let second = null;
      let cmp = null;

      if(sortDescriptor.column === 'unidadMedida.descripcion'){
        first = a.unidadMedida.descripcion as number;
        second = b.unidadMedida.descripcion as number;
        cmp = first < second ? -1 : first > second ? 1 : 0;
      }else{
        first = a[sortDescriptor.column as keyof IProductos] as number;
        second = b[sortDescriptor.column as keyof IProductos] as number;
        cmp = first < second ? -1 : first > second ? 1 : 0;
      }
  ...
  const renderCell = useCallback((row: any, columnKey: Key) => {
    ...
    switch (columnKey) {
      ...
      case 'precio':
        return (
          `$${Intl.NumberFormat('es-AR', { minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(row.precio)}`
        );
      case 'unidadMedida.descripcion':
        return (
          row.unidadMedida.descripcion
        );
      case "activo":
        return (
        ...
        return (
          <Dropdown>
            ...
            <DropdownMenu aria-label="Static Actions">
              <DropdownItem onPress={() => openUpdateProductoModal(row)} key="editar-producto">
              ...
              <DropdownItem onPress={() => activateInactivateProductoFnc(row)} key="alta-baja-producto">
                <div className="flex items-center">
                  {
