...
import { useImagenesStore, useUiStore } from "../../hooks";
...
export const ImagenesTable = () => {
  ...
  const openUpdateImagenModal = (imagen: IImagenes) => {
    setActiveImagen(imagen);
    toggleImagen();
  }

  // Activate/Inactivate - Imagen

  const activateInactivateImagenFnc = (imagen: IImagenes) => {
    ...
    activeInactiveImagen(dataUpdate);
  ...
  const pages = Math.ceil(imagenes.length / rowsPerPage);
  ...
  const filteredItems = useMemo(() => {
    ...
    let filteredElements: IImagenes[] = [...imagenes];
    ...
    filteredElements = filteredElements.filter((element: IImagenes) =>
      element.descripcion.includes(filterValue.toUpperCase())
    );

    return filteredElements;

  }, [imagenes, filterValue, filterActiveValue]);

  // Sort handler

  const sortedItems = useMemo(() => {
    return [...filteredItems].sort((a: IImagenes, b: IImagenes) => {
      const first = a[sortDescriptor.column as keyof IImagenes] as number;
      const second = b[sortDescriptor.column as keyof IImagenes] as number;
        ...
        return (
          <Dropdown>
            ...
            <DropdownMenu aria-label="Static Actions">
              <DropdownItem onPress={() => openUpdateImagenModal(row)} key="editar-imagen">
              ...
              <DropdownItem onPress={() => activateInactivateImagenFnc(row)} key="alta-baja-imagen">
                <div className="flex items-center">
                  {
                    row.activo ? <DislikeIcon className="w-4 h-4" /> : <LikeIcon className="w-4 h-4" />
                  }
                  <span className="ml-2">
                    {row.activo ? 'Baja de imagen' : 'Alta de imagen'}
                  </span>
                </div>
              </DropdownItem>
              <DropdownItem onPress={() => deleteImagen(row)} key="eliminar-imagen">
  ...
  return (
    ...
    >
      ...
      <TableBody
        emptyContent={"No se encontraron imagenes"}
